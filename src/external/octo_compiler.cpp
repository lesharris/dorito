...
void octo_list_destroy(octo_list *list, void items(void *)) {
  if (items)
    for (int z = 0; z < list->count; z++) items(list->data[z]);
  free(list->data);
}
...
octo_tok *octo_make_tok_null(int line, int pos) {
  ...
  return r->type = OCTO_TOK_EOF, r->line = line, r->pos = pos, r->str_value = (char *) "", r;
}
...
void octo_free_smode(octo_smode *x) {
  for (int z = 0; z < 256; z++)
    if (x->modes[z]) octo_free_macro(x->modes[z]);
  free(x);
}
...
char *octo_intern_counted(octo_program *p, char *name, int length) {
  ...
  if (p->strings_used + length + 1 >= OCTO_INTERN_MAX) {
    return p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX,
                                     (char *) "Internal Error: exhausted string interning table."), (char *) "";
  }
...
void octo_fetch_token(octo_program *p) {
  if (octo_is_end(p)) {
    p->is_error = 1;
    snprintf(p->error, OCTO_ERR_MAX, "Unexpected EOF.");
    return;
  }
  ...
  if (p->source[0] == '"') {
    ...
    while (1) {
      ...
      if (c == '\0') {
        p->is_error = 1;
        snprintf(p->error, OCTO_ERR_MAX, "Missing a closing \" in a string literal.");
      ...
      if (c == '\\') {
        ...
        if (ec == 't') str_buffer[index++] = '\t';
          ...
          p->is_error = 1;
          snprintf(p->error, OCTO_ERR_MAX, "Unrecognized escape character '%c' in a string literal.", ec);
      ...
      if (index >= 4095) {
        p->is_error = 1;
        snprintf(p->error, OCTO_ERR_MAX, "String literals must be < 4096 characters long.");
        p->error_line = p->source_line, p->error_pos = p->source_pos;
      }
    }
    str_buffer[index++] = '\0';
  ...
  p->current_token = octo_tok_copy(t);
...
octo_tok *octo_next(octo_program *p) {
  ...
  octo_tok *r = (octo_tok *) octo_list_remove(&p->tokens, 0);
  p->error_line = r->line, p->error_pos = r->pos;

  if (p->current_token)
    octo_free_tok(p->current_token);

  p->current_token = octo_tok_copy(r);
...
octo_tok *octo_peek(octo_program *p) {
  if (p->tokens.count == 0) octo_fetch_token(p);
  if (p->is_error) return octo_make_tok_null(p->source_line, p->source_pos);
  return (octo_tok *) octo_list_get(&p->tokens, 0);
}
...
int octo_peek_match(octo_program *p, char *name, int index) {
  ...
  octo_tok *t = (octo_tok *) octo_list_get(&p->tokens, index);
...
char *octo_reserved_words[] = {(char *) ":=", (char *) "|=", (char *) "&=", (char *) "^=", (char *) "-=", (char *) "=-",
                               (char *) "+=", (char *) ">>=", (char *) "<<=", (char *) "==", (char *) "!=",
                               (char *) "<", (char *) ">", (char *) "<=", (char *) ">=", (char *) "key",
                               (char *) "-key", (char *) "hex", (char *) "bighex", (char *) "random", (char *) "delay",
                               (char *) ":", (char *) ":next", (char *) ":unpack", (char *) ":breakpoint",
                               (char *) ":proto", (char *) ":alias", (char *) ":const", (char *) ":org", (char *) ";",
                               (char *) "return", (char *) "clear", (char *) "bcd", (char *) "save", (char *) "load",
                               (char *) "buzzer", (char *) "if", (char *) "then", (char *) "begin", (char *) "else",
                               (char *) "end", (char *) "jump", (char *) "jump0", (char *) "native", (char *) "sprite",
                               (char *) "loop", (char *) "while", (char *) "again", (char *) "scroll-down",
                               (char *) "scroll-up", (char *) "scroll-right", (char *) "scroll-left", (char *) "lores",
                               (char *) "hires", (char *) "loadflags", (char *) "saveflags", (char *) "i",
                               (char *) "audio", (char *) "plane", (char *) ":macro", (char *) ":calc",
                               (char *) ":byte", (char *) ":call", (char *) ":stringmode", (char *) ":assert",
                               (char *) ":monitor", (char *) ":pointer", (char *) "pitch",};

int octo_is_reserved(char *name) {
  for (size_t z = 0; z < sizeof(octo_reserved_words) / sizeof(char *); z++)
    if (strcmp(name, octo_reserved_words[z]) == 0) return 1;
  return 0;
}
...
int octo_check_name(octo_program *p, char *name, char *kind) {
  ...
  if (strncmp("OCTO_", name, 5) == 0 || octo_is_reserved(name)) {
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "The name '%s' is reserved and cannot be used for a %s.", name,
                              kind);
    return 0;
  }
  return 1;
...
int octo_register(octo_program *p) {
  ...
  octo_reg *a = (octo_reg *) octo_map_get(&p->aliases, t->str_value);
...
int octo_value_range(octo_program *p, int n, int mask) {
  if (mask == 0xF && (n < 0 || n > mask))
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Argument %d does not fit in 4 bits- must be in range [0,15].",
                              n);
  if (mask == 0xFF && (n < -128 || n > mask))
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX,
                              "Argument %d does not fit in a byte- must be in range [-128,255].", n);
  if (mask == 0xFFF && (n < 0 || n > mask))
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Argument %d does not fit in 12 bits.", n);
  if (mask == 0xFFFF && (n < 0 || n > mask))
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Argument %d does not fit in 16 bits.", n);
  return n & mask;
}

void octo_value_fail(octo_program *p, char *w, char *n, int undef) {
  if (p->is_error) return;
  if (octo_is_register(p, n))
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Expected %s value, but found the register %s.", w, n);
  else if (octo_is_reserved(n))
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Expected %s value, but found the keyword '%s'. Missing a token?",
                              w, n);
  else if (undef)
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Expected %s value, but found the undefined name '%s'.", w, n);
}
...
int octo_value_4bit(octo_program *p) {
  ...
  char *n = t->str_value;
  octo_free_tok(t);
  octo_const *c = (octo_const *) octo_map_get(&p->constants, n);
  if (c != NULL) return octo_value_range(p, c->value, 0xF);
  return octo_value_fail(p, (char *) "a 4-bit", n, 1), 0;
}
...
int octo_value_8bit(octo_program *p) {
  ...
  return octo_value_fail(p, (char *) "an 8-bit", n, 1), 0;
}
...
int octo_value_12bit(octo_program *p) {
  ...
  octo_value_fail(p, (char *) "a 12-bit", n, 0);
  if (p->is_error) return 0;
  if (!octo_check_name(p, n, (char *) "label"))
    return 0;
  octo_proto *pr = (octo_proto *) octo_map_get(&p->protos, n);
...
int octo_value_16bit(octo_program *p, int can_forward_ref, int offset) {
  ...
  octo_value_fail(p, (char *) "a 16-bit", n, 0);
  if (p->is_error) return 0;
  if (!octo_check_name(p, n, (char *) "label"))
    return 0;
  if (!can_forward_ref) {
...
octo_const *octo_value_constant(octo_program *p) {
  ...
  if (octo_map_get(&p->protos, n) != NULL)
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "A constant reference to '%s' may not be forward-declared.", n);
  return octo_value_fail(p, (char *) "a constant", n, 1), octo_make_const(0, 0);
}
...
void octo_macro_body(octo_program *p, char *desc, char *name, octo_macro *m) {
  ...
  octo_expect(p, (char *) "{");
  if (p->is_error) {
    snprintf(p->error, OCTO_ERR_MAX, "Expected '{' for definition of %s '%s'.", desc, name);
    return;
  }
  int depth = 1;
  while (!octo_is_end(p)) {
...
double octo_calc_terminal(octo_program *p, char *name) {
  ...
  if (octo_match(p, (char *) "PI"))
    return 3.141592653589793;
  if (octo_match(p, (char *) "E"))
    return 2.718281828459045;
  if (octo_match(p, (char *) "HERE"))
    return p->here;
  octo_tok *t = octo_next(p);
  if (t->type == OCTO_TOK_NUM) {
  ...
  if (octo_map_get(&p->protos, n) != NULL) {
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX,
                              "Cannot use forward declaration '%s' when calculating constant '%s'.", n, name);
    return 0;
  }
  octo_const *c = (octo_const *) octo_map_get(&p->constants, n);
  if (c != NULL) return c->value;
  if (strcmp(n, "(") != 0) {
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Found undefined name '%s' when calculating constant '%s'.", n,
                              name);
    return 0;
  }
...
double octo_calc_expr(octo_program *p, char *name) {
  // UNARY expression
  if (octo_match(p, (char *) "strlen"))
    return octo_interned_len(octo_string(p));
  if (octo_match(p, (char *) "-"))
    return -octo_calc_expr(p, name);
  if (octo_match(p, (char *) "~"))
    return ~((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "!"))
    return !((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "sin"))
    return sin(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "cos"))
    return cos(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "tan"))
    return tan(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "exp"))
    return exp(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "log"))
    return log(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "abs"))
    return fabs(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "sqrt"))
    return sqrt(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "sign"))
    return octo_sign(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "ceil"))
    return ceil(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "floor"))
    return floor(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "@"))
    return 0xFF & (p->rom[0xFFFF & ((int) octo_calc_expr(p, name))]);

  // expression BINARY expression
  double r = octo_calc_terminal(p, name);
  if (octo_match(p, (char *) "-"))
    return r - octo_calc_expr(p, name);
  if (octo_match(p, (char *) "+"))
    return r + octo_calc_expr(p, name);
  if (octo_match(p, (char *) "*"))
    return r * octo_calc_expr(p, name);
  if (octo_match(p, (char *) "/"))
    return r / octo_calc_expr(p, name);
  if (octo_match(p, (char *) "%"))
    return ((int) r) % ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "&"))
    return ((int) r) & ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "|"))
    return ((int) r) | ((int) octo_calc_expr(p, name));
  ...
  if (octo_match(p, (char *) ">>"))
    return ((int) r) >> ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "pow"))
    return pow(r, octo_calc_expr(p, name));
  if (octo_match(p, (char *) "min"))
    return octo_min(r, octo_calc_expr(p, name));
  if (octo_match(p, (char *) "max"))
    return octo_max(r, octo_calc_expr(p, name));
  if (octo_match(p, (char *) "<"))
    return r < octo_calc_expr(p, name);
  if (octo_match(p, (char *) ">"))
    return r > octo_calc_expr(p, name);
...
void octo_append(octo_program *p, char byte) {
  ...
  if (p->here > 0xFFFF) {
    p->is_error = 1;
    snprintf(p->error, OCTO_ERR_MAX, "ROM space is full.");
    return;
  }
  if (p->here > 0x200 && p->used[p->here]) {
    p->is_error = 1;
    snprintf(p->error, OCTO_ERR_MAX, "Data overlap. Address 0x%0X has already been defined.", p->here);
    return;
  }
...
void octo_instruction(octo_program *p, char a, char b) {
  uint8_t au = (uint8_t) a;
  uint8_t bu = (uint8_t) b;

  uint16_t data = (au << 8) | bu;

  p->listing->insert_or_assign((uint16_t) p->here, octo_listing_item{
      p->source_line,
      p->source_pos,
      p->here,
      data
...
void octo_resolve_label(octo_program *p, int offset) {
  ...
  char *n = octo_identifier(p, (char *) "label");
  if (p->is_error) return;
  ...
  if (octo_map_get(&p->aliases, n) != NULL) {
    p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "The name '%s' is already used by an alias.", n);
    return;
  }
  ...
  octo_proto *pr = (octo_proto *) octo_map_remove(&p->protos, n);
  for (int z = 0; z < pr->addrs.count; z++) {
    octo_pref *pa = (octo_pref *) octo_list_get(&pr->addrs, z);
    if (pa->is_long && (p->rom[pa->value] & 0xF0) == 0x60) { // :unpack long target
      ...
      p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Value 0x%0X for label '%s' does not fit in 12 bits.", target,
                                n);
      break;
...
void octo_compile_statement(octo_program *p) {
  ...
  if (octo_peek_is_register(p)) {
    ...
    if (octo_match(p, (char *) ":=")) {
      ...
      else if (octo_match(p, (char *) "random"))
        octo_instruction(p, 0xC0 | r, octo_value_8bit(p));
      else if (octo_match(p, (char *) "key"))
        octo_instruction(p, 0xF0 | r, 0x0A);
      else if (octo_match(p, (char *) "delay"))
        octo_instruction(p, 0xF0 | r, 0x07);
      else octo_instruction(p, 0x60 | r, octo_value_8bit(p));
    } else if (octo_match(p, (char *) "+=")) {
      ...
      octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x1);
    else if (octo_match(p, (char *) "&="))
      octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x2);
    else if (octo_match(p, (char *) "^="))
      octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x3);
    else if (octo_match(p, (char *) "=-"))
      octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x7);
    else if (octo_match(p, (char *) ">>="))
      octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x6);
    else if (octo_match(p, (char *) "<<="))
      octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0xE);
    else {
      octo_tok *t = octo_next(p);
      char d[256];
      if (!p->is_error)
        p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Unrecognized operator %s.", octo_tok_value(t, d));
      octo_free_tok(t);
    }
    return;
  }
  if (octo_match(p, (char *) ":"))
    octo_resolve_label(p, 0);
  else if (octo_match(p, (char *) ":next"))
    octo_resolve_label(p, 1);
  else if (octo_match(p, (char *) ":unpack")) {
    int a = 0;
    if (octo_match(p, (char *) "long")) {
      a = octo_value_16bit(p, 1, 0);
    } else {
      int v = octo_value_4bit(p);
      a = (v << 12) | octo_value_12bit(p);
    }
    octo_reg *rh = (octo_reg *) octo_map_get(&p->aliases, octo_intern(p, (char *) "unpack-hi"));
    octo_reg *rl = (octo_reg *) octo_map_get(&p->aliases, octo_intern(p, (char *) "unpack-lo"));
    octo_instruction(p, 0x60 | rh->value, a >> 8);
    octo_instruction(p, 0x60 | rl->value, a);
  } else if (octo_match(p, (char *) ":breakpoint"))
    p->breakpoints[p->here] = octo_string(p);
  else if (octo_match(p, (char *) ":monitor")) {
    char n[256];
  ...
  } else if (octo_match(p, (char *) ":assert")) {
    char *message = octo_peek_match(p, (char *) "{", 0) ? NULL : octo_string(p);
    if (!octo_calculated(p, (char *) "assertion")) {
      p->is_error = 1;
      if (message != NULL) snprintf(p->error, OCTO_ERR_MAX, "Assertion failed: %s", message);
      else
        snprintf(p->error, OCTO_ERR_MAX, "Assertion failed.");
    }
  } else if (octo_match(p, (char *) ":proto"))
    octo_free_tok(octo_next(p)); //deprecated
  else if (octo_match(p, (char *) ":alias")) {
    char *n = octo_identifier(p, (char *) "alias");
    if (octo_map_get(&p->constants, n) != NULL) {
      p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "The name '%s' is already used by a constant.", n);
      return;
    }
    int v = octo_peek_match(p, (char *) "{", 0) ? octo_calculated(p, (char *) "ANONYMOUS") : octo_register(p);
    if (v < 0 || v > 15) {
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "Register index must be in the range [0,F].");
      return;
    }
    octo_reg *prev = (octo_reg *) octo_map_set(&p->aliases, n, octo_make_reg(v));
    if (prev != NULL) octo_free_reg(prev);
  } else if (octo_match(p, (char *) ":byte")) {
    octo_append(p,
                octo_peek_match(p, (char *) "{", 0) ? (int) octo_calculated(p, (char *) "ANONYMOUS") : octo_value_8bit(
                    p));
  } else if (octo_match(p, (char *) ":pointer")) {
    int a = octo_peek_match(p, (char *) "{", 0) ? octo_calculated(p, (char *) "ANONYMOUS") : octo_value_16bit(p, 1, 0);
    octo_instruction(p, a >> 8, a);
  } else if (octo_match(p, (char *) ":org")) {
    p->here = (octo_peek_match(p, (char *) "{", 0) ? 0xFFFF & (int) octo_calculated(p, (char *) "ANONYMOUS")
                                                   : octo_value_16bit(p, 0, 0));
  } else if (octo_match(p, (char *) ":call")) {
    octo_immediate(p, 0x20, octo_peek_match(p, (char *) "{", 0) ? 0xFFF & (int) octo_calculated(p, (char *) "ANONYMOUS")
                                                                : octo_value_12bit(p));
  } else if (octo_match(p, (char *) ":const")) {
    char *n = octo_identifier(p, (char *) "constant");
    if (octo_map_get(&p->constants, n) != NULL) {
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "The name '%s' has already been defined.", n);
      return;
    }
    octo_map_set(&p->constants, n, octo_value_constant(p));
  } else if (octo_match(p, (char *) ":calc")) {
    char *n = octo_identifier(p, (char *) "calculated constant");
    octo_const *prev = (octo_const *) octo_map_get(&p->constants, n);
    if (prev != NULL && !prev->is_mutable) {
      p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "Cannot redefine the name '%s' with :calc.", n);
      return;
    }
    octo_map_set(&p->constants, n, octo_make_const(octo_calculated(p, n), 1));
    if (prev != NULL) octo_free_const(prev);
  } else if (octo_match(p, (char *) ";") || octo_match(p, (char *) "return"))
    octo_instruction(p, 0x00, 0xEE);
  else if (octo_match(p, (char *) "clear"))
    octo_instruction(p, 0x00, 0xE0);
  else if (octo_match(p, (char *) "bcd"))
    octo_instruction(p, 0xF0 | octo_register(p), 0x33);
  else if (octo_match(p, (char *) "delay"))
    octo_expect(p, (char *) ":="), octo_instruction(p, 0xF0 | octo_register(p), 0x15);
  else if (octo_match(p, (char *) "buzzer"))
    octo_expect(p, (char *) ":="), octo_instruction(p, 0xF0 | octo_register(p), 0x18);
  else if (octo_match(p, (char *) "pitch"))
    octo_expect(p, (char *) ":="), octo_instruction(p, 0xF0 | octo_register(p), 0x3A);
  else if (octo_match(p, (char *) "jump0"))
    octo_immediate(p, 0xB0, octo_value_12bit(p));
  else if (octo_match(p, (char *) "jump"))
    octo_immediate(p, 0x10, octo_value_12bit(p));
  else if (octo_match(p, (char *) "native"))
    octo_immediate(p, 0x00, octo_value_12bit(p));
  else if (octo_match(p, (char *) "audio"))
    octo_instruction(p, 0xF0, 0x02);
  else if (octo_match(p, (char *) "scroll-down"))
    octo_instruction(p, 0x00, 0xC0 | octo_value_4bit(p));
  else if (octo_match(p, (char *) "scroll-up"))
    octo_instruction(p, 0x00, 0xD0 | octo_value_4bit(p));
  else if (octo_match(p, (char *) "scroll-right"))
    octo_instruction(p, 0x00, 0xFB);
  else if (octo_match(p, (char *) "scroll-left"))
    octo_instruction(p, 0x00, 0xFC);
  else if (octo_match(p, (char *) "exit"))
    octo_instruction(p, 0x00, 0xFD);
  else if (octo_match(p, (char *) "lores"))
    octo_instruction(p, 0x00, 0xFE);
  else if (octo_match(p, (char *) "hires"))
    octo_instruction(p, 0x00, 0xFF);
  else if (octo_match(p, (char *) "sprite")) {
    int x = octo_register(p), y = octo_register(p);
    octo_instruction(p, 0xD0 | x, (y << 4) | octo_value_4bit(p));
  } else if (octo_match(p, (char *) "plane")) {
  ...
  } else if (octo_match(p, (char *) "saveflags"))
    octo_instruction(p, 0xF0 | octo_register(p), 0x75);
  else if (octo_match(p, (char *) "loadflags"))
    octo_instruction(p, 0xF0 | octo_register(p), 0x85);
  else if (octo_match(p, (char *) "save")) {
    int r = octo_register(p);
    if (octo_match(p, (char *) "-"))
      octo_instruction(p, 0x50 | r, (octo_register(p) << 4) | 0x02);
    else octo_instruction(p, 0xF0 | r, 0x55);
  } else if (octo_match(p, (char *) "load")) {
    int r = octo_register(p);
    if (octo_match(p, (char *) "-"))
      octo_instruction(p, 0x50 | r, (octo_register(p) << 4) | 0x03);
    else octo_instruction(p, 0xF0 | r, 0x65);
  } else if (octo_match(p, (char *) "i")) {
    if (octo_match(p, (char *) ":=")) {
      if (octo_match(p, (char *) "long")) {
        int a = octo_value_16bit(p, 1, 2);
        octo_instruction(p, 0xF0, 0x00);
        octo_instruction(p, (a >> 8), a);
      } else if (octo_match(p, (char *) "hex"))
        octo_instruction(p, 0xF0 | octo_register(p), 0x29);
      else if (octo_match(p, (char *) "bighex"))
        octo_instruction(p, 0xF0 | octo_register(p), 0x30);
      else octo_immediate(p, 0xA0, octo_value_12bit(p));
    } else if (octo_match(p, (char *) "+="))
      octo_instruction(p, 0xF0 | octo_register(p), 0x1E);
    else {
      octo_tok *t = octo_next(p);
      char d[256];
      p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "%s is not an operator that can target the i register.",
                                octo_tok_value(t, d));
      octo_free_tok(t);
    }
  } else if (octo_match(p, (char *) "if")) {
    int index = (octo_peek_match(p, (char *) "key", 1) || octo_peek_match(p, (char *) "-key", 1)) ? 2 : 3;
    if (octo_peek_match(p, (char *) "then", index)) {
      octo_conditional(p, 0), octo_expect(p, (char *) "then");
    } else if (octo_peek_match(p, (char *) "begin", index)) {
      octo_conditional(p, 1), octo_expect(p, (char *) "begin");
      octo_stack_push(&p->branches, octo_make_flow(p->here, p->source_line, p->source_pos, (char *) "begin"));
      octo_instruction(p, 0x00, 0x00);
    } else {
      for (int z = 0; z <= index; z++)
        if (!octo_is_end(p)) octo_free_tok(octo_next(p));
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "Expected 'then' or 'begin'.");
    }
  } else if (octo_match(p, (char *) "else")) {
    if (octo_stack_is_empty(&p->branches)) {
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "This 'else' does not have a matching 'begin'.");
      return;
    }
    octo_flow *f = (octo_flow *) octo_stack_pop(&p->branches);
    octo_jump(p, f->addr, p->here + 2);
    octo_free_flow(f);
    octo_stack_push(&p->branches, octo_make_flow(p->here, peek_line, peek_pos, (char *) "else"));
    octo_instruction(p, 0x00, 0x00);
  } else if (octo_match(p, (char *) "end")) {
    if (octo_stack_is_empty(&p->branches)) {
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "This 'end' does not have a matching 'begin'.");
      return;
    }
    octo_flow *f = (octo_flow *) octo_stack_pop(&p->branches);
    octo_jump(p, f->addr, p->here);
    octo_free_flow(f);
  } else if (octo_match(p, (char *) "loop")) {
    octo_stack_push(&p->loops, octo_make_flow(p->here, peek_line, peek_pos, (char *) "loop"));
    octo_stack_push(&p->whiles, octo_make_flow(-1, peek_line, peek_pos, (char *) "loop"));
  } else if (octo_match(p, (char *) "while")) {
    if (octo_stack_is_empty(&p->loops)) {
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "This 'while' is not within a loop.");
      return;
    }
    octo_conditional(p, 1);
    octo_stack_push(&p->whiles, octo_make_flow(p->here, peek_line, peek_pos, (char *) "while"));
    octo_immediate(p, 0x10, 0); // forward jump
  } else if (octo_match(p, (char *) "again")) {
    if (octo_stack_is_empty(&p->loops)) {
      p->is_error = 1;
      snprintf(p->error, OCTO_ERR_MAX, "This 'again' does not have a matching 'loop'.");
      return;
    }
    octo_flow *f = (octo_flow *) octo_stack_pop(&p->loops);
    octo_immediate(p, 0x10, f->addr);
    octo_free_flow(f);
    while (1) {
      octo_flow *f = (octo_flow *) octo_stack_pop(&p->whiles);
      int a = f->addr;
      octo_free_flow(f);
      if (a == -1) break;
      octo_jump(p, a, p->here);
    }
  } else if (octo_match(p, (char *) ":macro")) {
    char *n = octo_identifier(p, (char *) "macro");
    ...
    while (!octo_is_end(p) && !octo_peek_match(p, (char *) "{", 0))
      octo_list_append(&m->args, octo_identifier(p, (char *) "macro argument"));
    octo_macro_body(p, (char *) "macro", n, m);
  } else if (octo_match(p, (char *) ":stringmode")) {
    char *n = octo_identifier(p, (char *) "stringmode");
    if (octo_map_get(&p->stringmodes, n) == NULL) octo_map_set(&p->stringmodes, n, octo_make_smode());
    octo_smode *s = (octo_smode *) octo_map_get(&p->stringmodes, n);
    ...
    octo_macro_body(p, (char *) "string mode", n, m);
    for (int z = 0; z < octo_interned_len(alphabet); z++) {
      int c = 0xFF & alphabet[z];
      if (s->modes[c] != 0) {
        p->error_pos = alpha_base + z + (alpha_quote ? 1 : 0);
        p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "String mode '%s' is already defined for the character '%c'.",
                                  n, c);
        break;
      }
      s->values[c] = z;
    ...
    if (t->type == OCTO_TOK_NUM) {
      ...
      if (n < -128 || n > 255) {
        p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX,
                                  "Literal value '%d' does not fit in a byte- must be in range [-128,255].", n);
      }
      octo_append(p, n);
      return;
    }
    ...
    if (octo_map_get(&p->macros, n) != NULL) {
      octo_free_tok(octo_next(p));
      octo_macro *m = (octo_macro *) octo_map_get(&p->macros, n);
      octo_map bindings; // name -> tok
      octo_map_init(&bindings);
      octo_map_set(&bindings, octo_intern(p, (char *) "CALLS"), octo_make_tok_num(m->calls++));
      for (int z = 0; z < m->args.count; z++) {
        ...
        octo_map_set(&bindings, (char *) octo_list_get(&m->args, z), octo_next(p));
      }
      int splice_index = 0;
      for (int z = 0; z < m->body.count; z++) {
        octo_tok *t = (octo_tok *) octo_list_get(&m->body, z);
        octo_tok *r = (octo_tok *) ((t->type == OCTO_TOK_STR) ? octo_map_get(&bindings, (char *) t->str_value) : NULL);
      ...
      for (int tz = 0; tz < octo_interned_len(text); tz++) {
        ...
        if (s->modes[c] == 0) {
          ...
          p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "String mode '%s' is not defined for the character '%c'.",
                                    n, c);
          break;
        }
        octo_map bindings; // name -> tok
        octo_map_init(&bindings);
        octo_map_set(&bindings, octo_intern(p, (char *) "CALLS"), octo_make_tok_num(s->calls++)); // expansion count
        octo_map_set(&bindings, octo_intern(p, (char *) "CHAR"), octo_make_tok_num(c)); // ascii value of current char
        octo_map_set(&bindings, octo_intern(p, (char *) "INDEX"),
                     octo_make_tok_num((int) tz)); // index of char in input string
        octo_map_set(&bindings, octo_intern(p, (char *) "VALUE"),
                     octo_make_tok_num(s->values[c])); // index of char in class alphabet
        octo_macro *m = s->modes[c];
        for (int z = 0; z < m->body.count; z++) {
          octo_tok *t = (octo_tok *) octo_list_get(&m->body, z);
          octo_tok *r = (octo_tok *) ((t->type == OCTO_TOK_STR) ? octo_map_get(&bindings, t->str_value) : NULL);
          octo_list_insert(&p->tokens, octo_tok_copy(r == NULL ? t : r), splice_index++);
        }
        octo_map_destroy(&bindings, OCTO_DESTRUCTOR(octo_free_tok));
      }
...
octo_program *octo_program_init(char *text) {
  ...
  p->error_line = 0;
  p->error_pos = 0;
  if ((unsigned char) p->source[0] == 0xEF && (unsigned char) p->source[1] == 0xBB &&
      (unsigned char) p->source[2] == 0xBF)
    p->source += 3; // UTF-8 BOM
  octo_skip_whitespace(p);

#define octo_kc(l, n) (octo_map_set(&p->constants,octo_intern(p,((char *)"OCTO_KEY_" l)),octo_make_const(n,0)))
  octo_kc("1", 0x1), octo_kc("2", 0x2), octo_kc("3", 0x3), octo_kc("4", 0xC), octo_kc("Q", 0x4), octo_kc("W",
                                                                                                         0x5), octo_kc(
      "E", 0x6), octo_kc("R", 0xD), octo_kc("A", 0x7), octo_kc("S", 0x8), octo_kc("D", 0x9), octo_kc("F", 0xE), octo_kc(
      "Z", 0xA), octo_kc("X", 0x0), octo_kc("C", 0xB), octo_kc("V", 0xF);

  octo_map_set(&p->aliases, octo_intern(p, (char *) "unpack-hi"), octo_make_reg(0));
  octo_map_set(&p->aliases, octo_intern(p, (char *) "unpack-lo"), octo_make_reg(1));

  p->current_token = NULL;
  p->listing = new std::map<uint16_t, octo_listing_item>();
...
octo_program *octo_compile_str(char *text) {
  ...
  if (p->has_main) {
    octo_const *c = (octo_const *) octo_map_get(&p->constants, octo_intern(p, (char *) "main"));
    if (c == NULL)
      return p->is_error = 1, snprintf(p->error, OCTO_ERR_MAX, "This program is missing a 'main' label."), p;
  ...
  if (p->protos.keys.count > 0) {
    ...
    p->is_error = 1;
    snprintf(p->error, OCTO_ERR_MAX, "Undefined forward reference: %s", (char *) octo_list_get(&p->protos.keys, 0));
    return p;
  }
  if (!octo_stack_is_empty(&p->loops)) {
    octo_flow *f = (octo_flow *) octo_stack_pop(&p->loops);
    p->is_error = 1;
    snprintf(p->error, OCTO_ERR_MAX, "This 'loop' does not have a matching 'again'.");
  ...
  if (!octo_stack_is_empty(&p->branches)) {
    octo_flow *f = (octo_flow *) octo_stack_pop(&p->branches);
    p->is_error = 1;
    snprintf(p->error, OCTO_ERR_MAX, "This '%s' does not have a matching 'end'.", f->type);
    p->error_line = f->line, p->error_pos = f->pos;
    octo_free_flow(f);
    return p;
  }
