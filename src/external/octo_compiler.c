...
void octo_free_smode(octo_smode *x) {
  for (int z = 0; z < 256; z++)if (x->modes[z])octo_free_macro(x->modes[z]);
  free(x);
}
...
char *octo_reserved_words[] = {
    ...
    (char *) "<=", (char *) ">=", (char *) "key", (char *) "-key", (char *) "hex", (char *) "bighex", (char *) "random",
    (char *) "delay", (char *) ":", (char *) ":next", (char *) ":unpack",
    (char *) ":breakpoint", (char *) ":proto", (char *) ":alias", (char *) ":const", (char *) ":org", (char *) ";",
    (char *) "return", (char *) "clear", (char *) "bcd",
    (char *) "save", (char *) "load", (char *) "buzzer", (char *) "if", (char *) "then", (char *) "begin",
    (char *) "else", (char *) "end", (char *) "jump", (char *) "jump0",
    (char *) "native", (char *) "sprite", (char *) "loop", (char *) "while", (char *) "again", (char *) "scroll-down",
    (char *) "scroll-up", (char *) "scroll-right", (char *) "scroll-left",
    (char *) "lores", (char *) "hires", (char *) "loadflags", (char *) "saveflags", (char *) "i", (char *) "audio",
    (char *) "plane", (char *) ":macro", (char *) ":calc", (char *) ":byte",
    (char *) ":call", (char *) ":stringmode", (char *) ":assert", (char *) ":monitor", (char *) ":pointer",
    (char *) "pitch",
};
...
char *octo_identifier(octo_program *p, char *kind) {
  ...
  char *n = t->str_value;
  octo_free_tok(t);
  if (!octo_check_name(p, n, kind))return (char *) "";
  return n;
}
...
int octo_value_12bit(octo_program *p) {
  ...
  if (p->is_error)return 0;
  if (!octo_check_name(p, n, (char *) "label"))return 0;
...
double octo_calc_terminal(octo_program *p, char *name) {
  ...
  if (octo_match(p, (char *) "PI"))return 3.141592653589793;
  if (octo_match(p, (char *) "E"))return 2.718281828459045;
  if (octo_match(p, (char *) "HERE"))return p->here;
  octo_tok *t = octo_next(p);
  if (t->type == OCTO_TOK_NUM) {
...
double octo_calc_expr(octo_program *p, char *name) {
  // UNARY expression
  if (octo_match(p, (char *) "strlen")) return octo_interned_len(octo_string(p));
  if (octo_match(p, (char *) "-")) return -octo_calc_expr(p, name);
  if (octo_match(p, (char *) "~")) return ~((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "!")) return !((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "sin")) return sin(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "cos")) return cos(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "tan")) return tan(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "exp")) return exp(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "log")) return log(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "abs")) return fabs(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "sqrt")) return sqrt(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "sign")) return octo_sign(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "ceil")) return ceil(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "floor")) return floor(octo_calc_expr(p, name));
  if (octo_match(p, (char *) "@")) return 0xFF & (p->rom[0xFFFF & ((int) octo_calc_expr(p, name))]);

  // expression BINARY expression
  double r = octo_calc_terminal(p, name);
  if (octo_match(p, (char *) "-")) return r - octo_calc_expr(p, name);
  if (octo_match(p, (char *) "+")) return r + octo_calc_expr(p, name);
  if (octo_match(p, (char *) "*")) return r * octo_calc_expr(p, name);
  if (octo_match(p, (char *) "/")) return r / octo_calc_expr(p, name);
  if (octo_match(p, (char *) "%")) return ((int) r) % ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "&")) return ((int) r) & ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "|")) return ((int) r) | ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "^")) return ((int) r) ^ ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "<<")) return ((int) r) << ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) ">>")) return ((int) r) >> ((int) octo_calc_expr(p, name));
  if (octo_match(p, (char *) "pow")) return pow(r, octo_calc_expr(p, name));
  if (octo_match(p, (char *) "min")) return octo_min(r, octo_calc_expr(p, name));
  if (octo_match(p, (char *) "max")) return octo_max(r, octo_calc_expr(p, name));
  if (octo_match(p, (char *) "<")) return r < octo_calc_expr(p, name);
  if (octo_match(p, (char *) ">")) return r > octo_calc_expr(p, name);
  if (octo_match(p, (char *) "<=")) return r <= octo_calc_expr(p, name);
  if (octo_match(p, (char *) ">=")) return r >= octo_calc_expr(p, name);
...
void octo_compile_statement(octo_program *p) {
  ...
  if (octo_peek_is_register(p)) {
    ...
    if (octo_match(p, (char *) ":=")) {
      ...
      else if (octo_match(p, (char *) "random")) octo_instruction(p, 0xC0 | r, octo_value_8bit(p));
      else if (octo_match(p, (char *) "key")) octo_instruction(p, 0xF0 | r, 0x0A);
      else if (octo_match(p, (char *) "delay")) octo_instruction(p, 0xF0 | r, 0x07);
    ...
    } else if (octo_match(p, (char *) "|=")) octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x1);
    else if (octo_match(p, (char *) "&=")) octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x2);
    else if (octo_match(p, (char *) "^=")) octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x3);
    else if (octo_match(p, (char *) "=-")) octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x7);
    else if (octo_match(p, (char *) ">>=")) octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0x6);
    else if (octo_match(p, (char *) "<<=")) octo_instruction(p, 0x80 | r, (octo_register(p) << 4) | 0xE);
    else {
      octo_tok *t = octo_next(p);
      char d[256];
      if (!p->is_error)
  ...
  if (octo_match(p, (char *) ":"))octo_resolve_label(p, 0);
  else if (octo_match(p, (char *) ":next"))octo_resolve_label(p, 1);
  else if (octo_match(p, (char *) ":unpack")) {
    int a = 0;
    if (octo_match(p, (char *) "long")) { a = octo_value_16bit(p, 1, 0); }
    else {
  ...
  } else if (octo_match(p, (char *) ":breakpoint")) p->breakpoints[p->here] = octo_string(p);
  ...
  } else if (octo_match(p, (char *) ":proto"))octo_free_tok(octo_next(p));//deprecated
    ...
    octo_immediate(p, 0x20,
                   octo_peek_match(p, (char *) "{", 0) ? 0xFFF & (int) octo_calculated(p, (char *) "ANONYMOUS")
                                                       : octo_value_12bit(p));
  ...
  } else if (octo_match(p, (char *) ";") || octo_match(p, (char *) "return")) octo_instruction(p, 0x00, 0xEE);
  else if (octo_match(p, (char *) "clear")) octo_instruction(p, 0x00, 0xE0);
  else if (octo_match(p, (char *) "bcd")) octo_instruction(p, 0xF0 | octo_register(p), 0x33);
  ...
  else if (octo_match(p, (char *) "jump0")) octo_immediate(p, 0xB0, octo_value_12bit(p));
  else if (octo_match(p, (char *) "jump")) octo_immediate(p, 0x10, octo_value_12bit(p));
  else if (octo_match(p, (char *) "native")) octo_immediate(p, 0x00, octo_value_12bit(p));
  else if (octo_match(p, (char *) "audio")) octo_instruction(p, 0xF0, 0x02);
  else if (octo_match(p, (char *) "scroll-down")) octo_instruction(p, 0x00, 0xC0 | octo_value_4bit(p));
  else if (octo_match(p, (char *) "scroll-up")) octo_instruction(p, 0x00, 0xD0 | octo_value_4bit(p));
  else if (octo_match(p, (char *) "scroll-right")) octo_instruction(p, 0x00, 0xFB);
  else if (octo_match(p, (char *) "scroll-left")) octo_instruction(p, 0x00, 0xFC);
  else if (octo_match(p, (char *) "exit")) octo_instruction(p, 0x00, 0xFD);
  else if (octo_match(p, (char *) "lores")) octo_instruction(p, 0x00, 0xFE);
  else if (octo_match(p, (char *) "hires")) octo_instruction(p, 0x00, 0xFF);
  ...
  } else if (octo_match(p, (char *) "saveflags"))octo_instruction(p, 0xF0 | octo_register(p), 0x75);
  else if (octo_match(p, (char *) "loadflags"))octo_instruction(p, 0xF0 | octo_register(p), 0x85);
  else if (octo_match(p, (char *) "save")) {
    int r = octo_register(p);
    if (octo_match(p, (char *) "-")) octo_instruction(p, 0x50 | r, (octo_register(p) << 4) | 0x02);
    else octo_instruction(p, 0xF0 | r, 0x55);
  } else if (octo_match(p, (char *) "load")) {
    int r = octo_register(p);
    if (octo_match(p, (char *) "-")) octo_instruction(p, 0x50 | r, (octo_register(p) << 4) | 0x03);
    ...
    if (octo_match(p, (char *) ":=")) {
      if (octo_match(p, (char *) "long")) {
        ...
        octo_instruction(p, (a >> 8), a);
      } else if (octo_match(p, (char *) "hex")) octo_instruction(p, 0xF0 | octo_register(p), 0x29);
      else if (octo_match(p, (char *) "bighex")) octo_instruction(p, 0xF0 | octo_register(p), 0x30);
      else octo_immediate(p, 0xA0, octo_value_12bit(p));
    } else if (octo_match(p, (char *) "+=")) octo_instruction(p, 0xF0 | octo_register(p), 0x1E);
    else {
      octo_tok *t = octo_next(p);
      char d[256];
    ...
    if (octo_map_get(&p->macros, n) != NULL) {
      ...
      for (int tz = 0; tz < octo_interned_len(text); tz++) {
        ...
        octo_map_set(&bindings, octo_intern(p, (char *) "CHAR"),
                     octo_make_tok_num(c));            // ascii value of current char
