...
namespace dorito {
  void SpriteEditorWidget::Draw() {
    ...
    if (!ImGui::Begin(ICON_FA_PAINT_BRUSH " Sprite Editor", &UI::ShowSpriteEditor)) {
      ...
      static float scale = 24.0f;
      static uint8_t color = 1;
      static bool colorSprite = true;
      static bool hiresSprite = true;

      static std::vector<std::vector<uint8_t>> bitplanes{
          std::vector<uint8_t>(16 * 16),
          std::vector<uint8_t>(16 * 16)
      };

      static uint8_t spriteWidth = 16;
      static uint8_t spriteHeight = 16;
      ...
      auto ReadRow = [&](uint8_t plane, uint8_t row) {
      ...
      auto WriteRow = [&](uint8_t plane, uint8_t row, const std::vector<uint8_t> &data) {
        for (auto x = 0; x < 16; x++) {
          bitplanes[plane][row * spriteWidth + x] = data[x];
        }
      };

      auto ReadColumn = [&](uint8_t plane, uint8_t column) {
        std::vector<uint8_t> result;
      ...
      auto WriteColumn = [&](uint8_t plane, uint8_t column, const std::vector<uint8_t> &data) {
        for (auto y = 0; y < 16; y++) {
